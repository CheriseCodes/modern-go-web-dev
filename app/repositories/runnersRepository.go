...
func (rr RunnersRepository) GetRunner(runnerId string) (*models.Runner, *models.ResponseError) {
	...
	for rows.Next() { // for each row in the query result... (should by 1 row)
		err := rows.Scan(&id, &firstName, &lastName, &age, &isActive, &country, &personalBest, &seasonBest) // get these column values
		if err != nil {
			return nil, &models.ResponseError{
				Message: err.Error(),
				Status:  http.StatusInternalServerError,
...
func (rr RunnersRepository) GetAllRunners() ([]*models.Runner, *models.ResponseError) {
	...
	var age int
	var isActive bool
	for rows.Next() { // for each row in the query result...
		err := rows.Scan(&id, &firstName, &lastName, &age, &isActive, &country, &personalBest, &seasonBest) // get these column values
		if err != nil {
			return nil, &models.ResponseError{
				Message: err.Error(),
		...
		runner := &models.Runner{ // save the runners data in the Runners type
			ID:           id,
			FirstName:    firstName,
			LastName:     lastName,
			Age:          age,
		...
		runners = append(runners, runner) // add the runner to the runners list
	}
	if rows.Err() != nil {
		return nil, &models.ResponseError{
			Message: err.Error(),
...
func (rr RunnersRepository) GetRunnersByYear(year int) ([]*models.Runner, *models.ResponseError) {
	query := `
		SELECT runners.id, runners.first_name, runners.last_name, runners.age, 
			runners.is_active, runners.country, runners.personal_best,
			results.race_result
